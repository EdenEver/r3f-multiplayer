...
import { AnimationState, InstancedSkinnedMeshData } from "./instanced-skinned-mesh-handler"
...
export class InstancedAnimator {
  ...
  public removeInstance(id: string) {
    this.instancedAnimation.removeInstance(id)
  }
  ...
  public update(deltaTime: number) {
    ...
    for (let i = 0; i < this.instancedAnimation.instancesData.length; i++) {
      ...
      for (const [animationName, animation] of Object.entries(instanceData.animations)) {
        const animationDuration = this.instancedAnimation.animationsByName[animationName].duration
      ...
      // DO: fix this, translations etc should always be updated, but animations should be updated based on distance from camera (zoom since ortho)

      // const updateRate = lerp(
      //   this.minAnimationInterval ?? 0,
      //   this.maxAnimationInterval ?? 0,
  ...
  get instances() {
    return this.instancedAnimation.instancesData
  }

  private weightPrecision = 0.1

  private calculateGroupID(animations: Record<string, AnimationState>, distance: number): string {
    let groupID = ""
    const animationNames = Object.keys(animations).sort()
    const maxAnimationIntervalSeconds = (this.maxAnimationInterval ?? 0) / 1000
