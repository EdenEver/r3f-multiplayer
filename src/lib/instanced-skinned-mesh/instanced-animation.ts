...
type SimpleSkinnedMesh = THREE.SkinnedMesh<THREE.BufferGeometry, THREE.Material>
...
export class InstancedAnimation {
  ...
  instanceMatrixNeedsUpdate() {
    for (let i = 0; i < this.instancedSkinnedMeshes.length; i++) {
      this.instancedSkinnedMeshes[i].instancedMesh.instanceMatrix.needsUpdate = true
    }
  }
  ...
  update(deltaTime: number) {
    for (let i = 0; i < this.instancesData.length; i++) {
      ...
      for (const [animationName, animation] of Object.entries(instanceData.animations)) {
        animation.time = (animation.time + deltaTime) % this.animationsByName[animationName].duration
      }
