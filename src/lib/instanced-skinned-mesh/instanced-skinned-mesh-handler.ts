...
export class InstancedSkinnedMeshHandler {
  ...
  constructor({
    ...
    this.instancedMesh = new InstancedSkinnedMesh(this.skinnedMesh.geometry, this.skinnedMesh.material, count)

    // @ts-expect-error - do: investigate this
    this.instancedMesh.copy(this.skinnedMesh)
    this.instancedMesh.bind(this.skinnedMesh.skeleton, this.skinnedMesh.bindMatrix)
    ...
    this.animations.forEach((clip, index) => {
      ...
      newClip.tracks.forEach((track) => {
        const interpolantBone = gltf.scene.getObjectByName(track.name.split(".")[0])
        // @ts-expect-error - do: investigate this
  ...
  // NOTE: make use of `maxLevelOfDetail`
